...
function renderCanvas(canvas) {
    ...
    canvas.width = size;
    canvas.height = size;
    canvas.style.left = (window.innerWidth - size);
    if (window.innerHeight > size) canvas.style.top = (window.innerHeight - size);
    ctx.fillRect(0, 0, size, size);
    var cells = this.cells;
    var cellWidth = size / cells.length;
    var cellHeight = size / cells.length;
