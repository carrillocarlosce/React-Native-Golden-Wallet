...
export default class ConfirmStore {
  @observable unconfirmTransaction = {}
  ...
  @computed get formatedDolar() {
    // TODO getRate
    const amountDolar = this.value.times(this.rate)
    return `$${Helper.formatUSD(amountDolar, false, 1000000, 2)}`
  }
  _onShowAdvance() {
    const formatedGasPrice = this.gasPrice.div(1e+9).toFixed(0)

    MainStore.dapp.advanceStore.setGasLimit(this.gasLimit.toString(10))
    MainStore.dapp.advanceStore.setGasPrice(formatedGasPrice)
  }
