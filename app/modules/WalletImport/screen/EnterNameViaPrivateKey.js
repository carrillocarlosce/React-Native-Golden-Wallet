...
@observer
export default class EnterNameViaPrivateKey extends Component {
    static propTypes = {
    ...
    renderErrorField = () => {
        const { isErrorTitle } = this.importPrivateKeyStore
        if (isErrorTitle) {
    ...
    render() {
        const { title, loading, isReadyCreate } = this.importPrivateKeyStore
        return (
